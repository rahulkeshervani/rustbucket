...
/// often composed of several smaller messages known as "frames". The purpose
/// of `Connection` is to read and write frames on the underlying `TcpStream`.
///
/// To read frames, the `Connection` uses an internal buffer, which is filled
/// up until there are enough bytes to create a full frame. When this happens,
/// the `Connection` creates the frame and returns it.
///
...
impl Connection {
    ...
    fn parse_frame(&mut self) -> crate::Result<Option<Frame>> {
        ...
        match Frame::check(&mut buf) {
            Ok(_) => {
                ...
                // end of the frame. Since the cursor had been eagerly advanced,
                // we reset the position to 0 before calling `parse`.
            ...
            // socket. Reading from the socket will be done in the loop in
            // `read_frame`.
    ...
    ///
    /// The `Frame` + End of Line is written to the socket.
    #[async_recursion]
    pub async fn write_frame(&mut self, frame: &Frame) -> std::io::Result<()> {
        match frame {
            Frame::Array(val) => {
                // Encode the array prefix.
    ...
    async fn write_value(&mut self, frame: &Frame) -> std::io::Result<()> {
        match frame {
            ...
            // recursion. Mini-redis has not needed this capability, so we
            // simply error.
